...
model's tooling; it buys a capability only in the cases below.

{{harness-choices}}
Everything else is pi's: bulk implementation, long reading, investigation.
Where another harness reaches the same model, running the child there adds a
process, not a capability.

One real difference, for the rare case it matters: pi children run in-process,
so a hung one does not surface as a failed exit — it simply never settles. The
other harnesses are separate processes or SDK sessions. Needing that isolation
is a reason to leave the harness. Being able to leave it is not.
